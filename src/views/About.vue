<template>
  <div class="px-6 md:px-10 bg-black min-h-screen">
  <!-- stanga -->
      <div class="flex flex-wrap">
      <div>
          <div class="w-full">
              <h1 class="pt-24 text-white text-2xl uppercase">I. Metode de contact:</h1>
          </div>
          <div class="mt-13 flex w-full">
              <a class="text-white mt-4 text-2xl mr-6" href="https://www.instagram.com/vlad.istrate01/">1) Profilul de Instagram:</a>
              <a target="_blank" href="https://www.instagram.com/vlad.istrate01/"><img src="@/assets/images/insta.svg"></a>
          </div>
          <div class="mt-16 flex w-full">
              <a  class="text-white mt-4 text-2xl mr-6">2) Profilul de Facebook:</a>
              <a target="_blank" href="https://www.facebook.com/profile.php?id=100008659219292"><img src="@/assets/images/face.svg"></a>
         </div>
         <div class="mt-20 flex w-full">
              <a  class="text-white mt-4 text-2xl mr-13">3) Profilul de Github:</a>
              <a target="_blank" href="https://github.com/FatKappa"><img src="@/assets/images/git.svg"></a>
         </div>
      </div>
<div class="w-1/4 min-h-screen ml-4">

    <h1 class="text-white text-2xl mt-24 uppercase">II. Surse de inspiratie:</h1>
    <div class="flex">
        <a class="text-white text-2xl mt-16 mr-4" target="_blank">1) Vue.Js:</a><a href="https://vuejs.org">
        <img src="@/assets/images/logo.png" class="h-16 w-16 mt-13"></a>
    </div>
    <div class="flex">
        <a class="text-white text-2xl mt-20 mr-4" target="_blank">2) Tailwindcss:</a><a href="https://tailwindcss.com">
        <img src="@/assets/images/tcss.png" class="h-16 w-16 mt-16 rounded-full"></a>
    </div>
    <div class="flex">
        <a class="text-white text-2xl mt-24 mr-4" target="_blank">3) Github:</a><a href="https://github.com">
        <img src="@/assets/images/git.svg" class="h-16 w-16 mt-20"></a>
    </div>
</div>
          <div class="w-5/12 min-h-screen ml-4">
    <div class="mt-24 text-2xl text-white">
    <h1 class="uppercase">III. Scurt istoric</h1>
    <p class="text-base mt-12 ml-4">In curand</p>
    </div>
  </div>
      </div>
</div>
</template>
<style>

</style>
